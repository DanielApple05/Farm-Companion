...
const LivestockDetail = () => {
  ...
  const fetchLivestock = async () => {
    try {
      ...
      const tips =
        response.data.livestockTips?.livestockTips || response.data.livestockTips || [];
  ...
  const vaccinations = livestock
    ? [...(livestock.vaccinations || [])].sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate))
    : [];
  const healthLogs = livestock
    ? [...(livestock.healthLogs || [])].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    : [];
        ...
        <main className="w-full px-4 sm:px-6 pb-24 pt-4 xl:pb-6 bg-gray-50 space-y-4 sm:space-y-6 xl:mt-20 mt-28 xl:mb-0 mb-20">
          ...
          {!loading && !livestock && (
            <div className="max-w-2xl mx-auto flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-sm text-gray-600 mt-2">{message || "No livestock data found."}</p>
              <button
                type="button"
          ...
          {!loading && livestock && (
            <>
              ...
              <section className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h1 className="text-xl font-semibold text-gray-900 truncate">{livestock.type}</h1>
                      <p className="text-sm text-gray-500 flex items-start gap-1 mt-1">
                        <MapPin size={12} className="mt-0.5 shrink-0" />
                        <span className="truncate">
                          {livestock.farm?.name}
                          {livestock.farm?.location && ` · ${livestock.farm.location}`}
                        </span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2">
                    <span
                      className={`text-xs px-2.5 py-1.5 rounded-md ${statusStyles[livestock.status] || "bg-gray-50 text-gray-600"
                        }`}
                    >
                      {livestock.status}
                    </span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mt-5 pt-4 border-t border-gray-100">
                  ...
                  <div className="bg-gray-50 sm:bg-transparent rounded-lg p-3 sm:p-0">
                    ...
                    <p className="text-sm font-medium text-gray-900 mt-1">{livestock.headcount} heads</p>
                  </div>
              ...
              <section className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="font-medium text-gray-900">Farm Advice</h2>
                    </div>
                    <p className="text-xs text-gray-400 mt-2">
                      Tips based on your {livestock.type?.toLowerCase()}s current stage.
                    </p>
                  </div>
                ...
                {livestockTips.length === 0 ? (
                  <div className="bg-gray-50 rounded-xl p-5 text-center">
                    <Lightbulb size={20} className="mx-auto text-gray-300 mb-2" />
                    <p className="text-xs text-gray-400">No advice available for this stage yet.</p>
                  </div>
                ) : (
                    ...
                    {livestockTips.map((tip, index) => (
                      <div key={index} className="flex items-start gap-3 bg-gray-50 rounded-xl p-3">
                        <div className="w-6 h-6 shrink-0 rounded-full bg-green-100 flex items-center justify-center">
                          <span className="text-xs text-green-700 font-medium">{index + 1}</span>
                        </div>
              ...
              <section className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Keep track of vaccinations and scheduled care.</p>
                  </div>
                ...
                {vaccinations.length === 0 ? (
                  <div className="bg-gray-50 rounded-xl p-4 text-center">
                    <Syringe size={20} className="mx-auto text-gray-300 mb-2" />
                    ...
                    {vaccinations.map((v, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-start gap-3 bg-gray-50 rounded-xl p-3">
                          ...
                          {!v.completedOn && (
                            <span
                              className={`shrink-0 text-[11px] px-2 py-1 rounded-md ${isOverdue ? "bg-red-50 text-red-700" : "bg-gray-100 text-gray-500"
                                }`}
                            >
              ...
              <section className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5">
                <div className="mb-4">
                  <h2 className="font-medium text-gray-900">Health Log</h2>
                  <p className="text-xs text-gray-400 mt-1">Track symptoms and AI-assisted health observations.</p>
                </div>
