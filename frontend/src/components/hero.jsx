...
const Hero = ({
  ...
  imageUrl = "/images/dashboardHero.png", // drop in your generated hero image URL here
}) => {

  const navigate = useNavigate();

  return (
    <section className="relative overflow-hidden rounded-b-2xl bg-green-50 h-80">
      <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-6 pl-8 h-full">
        {/* Text side */}
        <div className="max-w-lg h-full">
          <span className="inline-block text-xs font-medium text-green-700 bg-white px-3 py-1 rounded-full mb-4">
            {eyebrow}
          </span>
          ...
          <div className="grid grid-cols-3 gap-4">
            {stats.map(({ label, value, icon: Icon, tint }) => (
              <div key={label} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
                <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${tint}`}>
                  <Icon size={20} />
                </div>
                <div>
